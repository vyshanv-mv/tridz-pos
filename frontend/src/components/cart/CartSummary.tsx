  ...
  return (
    <div className="border-t border-border pt-4 space-y-2 text-sm text-muted-foreground">
      <div className="flex justify-between items-center">
        ...
        <span className="text-foreground">{formatCurrency(subtotal, profile?.currency)}</span>
      </div>

      {taxBreakdown.map((tax, index) => (
        <div key={index} className="flex justify-between items-center">
          <span className="font-medium text-foreground">{tax.title} ({tax.rate}%):</span>
          <span className="text-foreground">{formatCurrency(tax.amount, profile?.currency)}</span>
        </div>
      ))}

      <div className="flex justify-between font-bold text-lg pt-2 border-t border-border text-foreground">
        <span>Grand Total:</span>
        <span className="text-primary">{formatCurrency(grandTotal, profile?.currency)}</span>
      </div>
    </div>
  )
}
